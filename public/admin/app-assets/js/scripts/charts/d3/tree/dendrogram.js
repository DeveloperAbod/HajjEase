...
$(window).on("load", function () {
  var ele = d3.select("#dendrogram"),
    ...
    width =
      ele.node().getBoundingClientRect().width - margin.left - margin.right,
  ...
  d3.json(
    "/admin/app-assets/data/d3/tree/dendrogram.json",
    function (error, root) {
      ...
      var linkGroup = svg.append("g").attr("class", "d3-tree-link-group");
      ...
      var nodeGroup = svg.append("g").attr("class", "d3-tree-node-group");
      ...
      function resize() {
        // Layout variables
        (width =
          ele.node().getBoundingClientRect().width -
          margin.left -
          margin.right),
