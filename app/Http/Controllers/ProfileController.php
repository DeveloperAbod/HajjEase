...
class ProfileController extends Controller
{
    ...
    public function changePassword()
    {
        ...
        if (!Hash::check(
            ...
            return redirect("/profile")->with('icon', 'error')->with('msg', 'كلمة المرور الحالية غير صحيحة');
        }

        $user->password = Hash::make(request()->new_password);
        $user->save();

        return redirect("/profile")->with('icon', 'success')->with('msg', 'تم تغيير كلمة المرور بنجاح');
    }
    ...
    public function update()
    {
        ...
        request()->validate([
            ...
            'phone' => ['numeric', 'digits:9', 'required', Rule::unique('users')->ignore($user->id)] */
        ]);
        ...
        $user->save();
        return redirect("/profile")->with('icon', 'success')->with('msg', 'تم تعديل الملف الشخصي بنجاح');
    }
}
